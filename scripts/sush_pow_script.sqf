/*
	...
	Description:
	From the designated unit, it "creates" a host that players can free.

	Parameter(s):
	0: OBJECT - Unit name
	1 (optional): STRING - host "name"

	Returns:
	Nothing
	
	Example:
	0 = [this,"Hostage 1"] execVM "sush_pow_script.sqf";
*/
...
sush_pow_fn_addActionGetOut = {
	...
	private _actTxt = format ["<t color='#ff8133'>Get out of the vehicle!</t>", name _p];
...
sush_pow_fn_followAction = {
		...
		private _actTxt = format ["<t color='#ff8133'>Get into the vehicle!</t>", name _p];
...
//--> Main
//player createDiaryRecord ["Diary", ["Hostages "," All actions related to hostages can be found in the action menu under the scroll. Available actions: <br/> -Free <br/> -Follow <br/> -Wait <br/> -Enable (available on a two-wheeled vehicle, the hostage cannot be in 'Wait' mode) <br/> -Disconnect <br/> <br/> The freed hostages dynamically assume the player's position "(prone, crouch, stand)"]];
...
private _actionId = _pow addAction ["<t color='#ff8133'>Release hostage</t>",sush_pow_fn_takeAction,nil,6,true,true,"","_this distance _target < 3"];
...
while {true} do {
	...
	switch (_pow_state) do {
		case 1: {
			_pow setUserActionText [_actionId, "<t color='#ff8133'>Stay here!</t>"];
		};
		case 2: {
